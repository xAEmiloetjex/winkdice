...
async function main() {
  try {
    console.log('Building and packaging ../../../observable/index.js...');
    try {
      await execAsync('yarn build && npm pack', rxjsObservableRoot);
    } catch (err) {
      console.error('❌ Failed to build and package ../../../observable/index.js!');
      console.error(err);
      throw err;
    }
    ...
    for (const fixtureName of FIXTURES) {
      ...
      try {
        ...
        console.log('Setting custom dependency resolution for local ../../../observable/index.js...');
        addYarnResolution(fixturePath, '../../../observable/index.js', `file:${path.relative(fixturePath, rxjsObservableTgzPath)}`);
        ...
        try {
          ...
          removeYarnResolution(fixturePath, '../../../observable/index.js');
        } catch (err) {
