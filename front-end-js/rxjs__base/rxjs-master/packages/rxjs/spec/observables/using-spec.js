...
describe('using', () => {
    it('should dispose of the resource when the subscription is disposed', (done) => {
        let disposed = false;
        const source = using(() => new Subscription(() => disposed = true), (resource) => range(0, 3))
            .pipe(take(2));
        source.subscribe();
        if (disposed) {
            done();
    ...
    it('should accept factory returns promise resolves', (done) => {
        const expected = 42;
        let disposed = false;
        const e1 = using(() => new Subscription(() => disposed = true), (resource) => new Promise((resolve) => { resolve(expected); }));
        e1.subscribe({ next: x => {
    ...
    it('should accept factory returns promise rejects', (done) => {
        const expected = 42;
        let disposed = false;
        const e1 = using(() => new Subscription(() => disposed = true), (resource) => new Promise((resolve, reject) => { reject(expected); }));
        e1.subscribe({ next: x => {
                done(new Error('should not be called'));
            }, error: (x) => {
                expect(x).to.equal(expected);
    ...
    it('should raise error when observable factory throws', (done) => {
        const error = 'error';
        let disposed = false;
        const source = using(() => new Subscription(() => disposed = true), (resource) => {
            throw error;
        });
        source.subscribe({ next: (x) => {
                done(new Error('should not be called'));
