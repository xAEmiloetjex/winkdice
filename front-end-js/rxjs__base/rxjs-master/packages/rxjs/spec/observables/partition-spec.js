...
describe('partition', () => {
    ...
    it('should partition an observable of integers into even and odd', () => {
        rxTestScheduler.run(({ hot, expectSubscriptions }) => {
            ...
            const result = partition(e1, (x) => x % 2 === 1);
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const unsub = '    -------!          ';
            const e1Pipe = e1.pipe(mergeMap((x) => of(x)));
            const result = partition(e1Pipe, (x) => x === 'a');
    ...
    it('should accept thisArg', () => {
        const thisArg = {};
        partition(of(1), function (value) {
            expect(this).to.deep.equal(thisArg);
            return true;
        }, thisArg).forEach((observable) => observable.subscribe());
    });
});
