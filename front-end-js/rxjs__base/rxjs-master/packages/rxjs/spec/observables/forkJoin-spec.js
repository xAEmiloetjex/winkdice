...
describe('forkJoin', () => {
    ...
    it('should support a resultSelector with an Array of ObservableInputs', () => {
        const results = [];
        forkJoin([of(1, 2, 3), of(4, 5, 6), of(7, 8, 9)], (a, b, c) => a + b + c).subscribe({
            next(value) {
                results.push(value);
            },
            error(err) {
    ...
    it('should finalize in the proper order', () => {
        const results = [];
        const source = forkJoin([1, 2, 3, 4].map((n) => timer(100, rxTestScheduler).pipe(map(() => n), finalize(() => results.push(`finalized ${n}`)))));
