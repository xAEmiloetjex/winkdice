...
describe('onErrorResumeNext', () => {
    ...
    it('should skip invalid sources and move on', () => {
        const results = [];
        onErrorResumeNext(of(1), [2, 3, 4], { notValid: 'LOL' }, of(5, 6)).subscribe({
    ...
    it('should call finalize after each sync observable', () => {
        const results = [];
        onErrorResumeNext(of(1).pipe(finalize(() => results.push('finalize 1'))), of(2).pipe(finalize(() => results.push('finalize 2'))), of(3).pipe(finalize(() => results.push('finalize 3'))), of(4).pipe(finalize(() => results.push('finalize 4')))).subscribe({
            next: (value) => results.push(value),
            complete: () => results.push('complete'),
        });
