...
describe('mergeScan', () => {
    ...
    it('should mergeScan things', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(mergeScan((acc, x) => of(acc.concat(x)), []));
    ...
    it('should mergeScan values and be able to asynchronously project them', () => {
        testScheduler.run(({ hot, time, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(mergeScan((acc, x) => of(acc.concat(x)).pipe(delay(t)), []));
            expectObservable(result).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        testScheduler.run(({ hot, time, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(mergeMap((x) => of(x)), mergeScan((acc, x) => of(acc.concat(x)).pipe(delay(t)), []), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should stop listening to a synchronous observable when unsubscribed', () => {
        ...
        of(null)
            .pipe(mergeScan(() => synchronousObservable, 0), takeWhile((x) => x != 2) // unsubscribe at the second side-effect
        )
            .subscribe(() => {
            /* noop */
        });
    ...
    it('should handle errors in the projection function', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(mergeScan((acc, x) => {
                if (x === 'd') {
                    throw new Error('bad!');
                }
    ...
    it('should stop listening to a synchronous observable when unsubscribed', () => {
        ...
        synchronousObservable
            .pipe(mergeScan((acc, value) => of(value), 0), take(3))
            .subscribe(() => {
            /* noop */
        });
