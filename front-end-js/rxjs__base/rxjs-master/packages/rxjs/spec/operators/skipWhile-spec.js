...
describe('skipWhile', () => {
    ...
    it('should skip all elements with a truthy predicate', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const sourceSubs = '   ^---------------!';
            const expected = '     ----------------|';
            const result = source.pipe(skipWhile(() => {
                return {};
            }));
            expectObservable(result).toBe(expected);
            expectSubscriptions(source.subscriptions).toBe(sourceSubs);
    ...
    it('should not skip any elements with a falsy predicate', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const sourceSubs = '   ^---------------!';
            const expected = '     -2--3--4--5--6--|';
            const result = source.pipe(skipWhile(() => undefined));
            expectObservable(result).toBe(expected);
            expectSubscriptions(source.subscriptions).toBe(sourceSubs);
        });
    });
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const result = source.pipe(mergeMap(function (x) {
                return of(x);
    ...
    it('should invoke predicate while its false and never again', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const result = source.pipe(skipWhile(predicate), tap({
                complete() {
    ...
    it('should stop listening to a synchronous observable when unsubscribed', () => {
        ...
        synchronousObservable
            .pipe(skipWhile((value) => value < 2), take(1))
            .subscribe(() => {
            /* noop */
        });
