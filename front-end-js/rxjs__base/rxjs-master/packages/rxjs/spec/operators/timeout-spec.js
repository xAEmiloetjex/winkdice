...
describe('timeout operator', () => {
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions, time }) => {
            ...
            const expected = '--a--b--c--        ';
            const unsub = '   ----------!        ';
            const result = e1.pipe(mergeMap((x) => of(x)), timeout(t, rxTestScheduler), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    describe('config', () => {
        ...
        it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
            rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions, time }) => {
                ...
                const expected = '--a--b--c--        ';
                const unsub = '   ----------!        ';
                const result = e1.pipe(mergeMap((x) => of(x)), timeout({ each: t }), mergeMap((x) => of(x)));
                expectObservable(result, unsub).toBe(expected);
                expectSubscriptions(e1.subscriptions).toBe(e1subs);
            });
        });
    ...
    describe('using with', () => {
        ...
        it('should not break unsubscription chain when unsubscribed explicitly', () => {
            rxTestScheduler.run(({ hot, cold, time, expectObservable, expectSubscriptions }) => {
                ...
                const expected = '    ---a---b----x--    ';
                const unsub = '       --------------!    ';
                const result = source.pipe(mergeMap((x) => of(x)), timeout({ each: t, with: () => inner }), mergeMap((x) => of(x)));
                expectObservable(result, unsub).toBe(expected);
                expectSubscriptions(source.subscriptions).toBe(sourceSubs);
                expectSubscriptions(inner.subscriptions).toBe(innerSubs);
            });
