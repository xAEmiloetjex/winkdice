...
describe('switchAll', () => {
    ...
    it('should unsub inner observables', () => {
        const unsubbed = [];
        of('a', 'b')
            .pipe(map((x) => new Observable((subscriber) => {
            subscriber.complete();
            return () => {
                unsubbed.push(x);
            };
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        testScheduler.run(({ hot, cold, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '--------a---b----            ';
            const unsub = '   ----------------!            ';
            const result = e1.pipe(mergeMap((x) => of(x)), switchAll(), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(x.subscriptions).toBe(xsubs);
            expectSubscriptions(y.subscriptions).toBe(ysubs);
        });
    ...
    it('should not leak when child completes before each switch (prevent memory leaks #2355)', () => {
        let iStream;
        const oStreamControl = new Subject();
        const oStream = oStreamControl.pipe(map(() => (iStream = new Subject())));
    ...
    it('should not leak if we switch before child completes (prevent memory leaks #2355)', () => {
        const oStreamControl = new Subject();
        const oStream = oStreamControl.pipe(map(() => new Subject()));
        const switcher = oStream.pipe(switchAll());
        const result = [];
        const subscription = switcher.subscribe((x) => result.push(x));
        [0, 1, 2, 3, 4].forEach((n) => {
