...
describe('Observable.prototype.buffer', () => {
    ...
    it('should not break unsubscription chains when unsubscribed explicitly', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const result = a.pipe(mergeMap((x) => of(x)), buffer(b), mergeMap((x) => of(x)));
    ...
    it('should buffer when Promise resolves', (done) => {
        ...
        e1.pipe(buffer(new Promise((resolve) => setTimeout(() => resolve(), 8)))).subscribe({
            next: (x) => {
        ...
        cases.forEach(({ source, notifier }, index) => {
            it(`should be equivalent for case ${index}`, () => {
                testScheduler.run(({ hot, expectObservable }) => {
                    ...
                    expectObservable(a.pipe(buffer(b))).toEqual(a.pipe(window(b), mergeMap((w) => w.pipe(toArray()))));
                });
            });
        });
    });
