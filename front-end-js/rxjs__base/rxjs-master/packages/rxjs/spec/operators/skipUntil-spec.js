...
describe('skipUntil', () => {
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '  ----------          ';
            const unsub = '     ---------!          ';
            const result = e1.pipe(mergeMap((x) => of(x)), skipUntil(skip), mergeMap((x) => of(x)));
    ...
    it('should not break unsubscription chains with interop inners when result is unsubscribed explicitly', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(mergeMap((x) => of(x)), skipUntil(asInteropObservable(skip)), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
            expectSubscriptions(skip.subscriptions).toBe(skipSubs);
        });
    ...
    it('should stop listening to a synchronous notifier after its first nexted value', () => {
        const sideEffects = [];
        const synchronousNotifier = concat(defer(() => {
            sideEffects.push(1);
            return of(1);
        }), defer(() => {
            sideEffects.push(2);
    ...
    it('should skip until Promise resolves', (done) => {
        ...
        e1.pipe(skipUntil(new Promise((resolve) => setTimeout(() => resolve(), 8)))).subscribe({
            next: (x) => {
