...
describe('finalize', () => {
    ...
    it('should finalize sources before sinks', () => {
        const finalized = [];
        of(42)
            .pipe(finalize(() => finalized.push('source')), finalize(() => finalized.push('sink')))
            .subscribe();
    ...
    it('should finalize in the proper order', () => {
        const results = [];
        of(1)
            .pipe(finalize(() => results.push(1)), finalize(() => results.push(2)), finalize(() => results.push(3)), finalize(() => results.push(4)))
            .subscribe();
