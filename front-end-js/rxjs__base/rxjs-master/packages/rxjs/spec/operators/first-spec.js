...
describe('first', () => {
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '   ----               ';
            const unsub = '      ---!               ';
            const result = e1.pipe(mergeMap((x) => of(x)), first(), mergeMap((x) => of(x)));
    ...
    it('should support type guards without breaking previous behavior', () => {
        // type guards with interfaces and classes
        {
            ...
            const foobar = new Foo(); // type is the interface, not the class
            of(foobar)
                .pipe(first())
                .subscribe((x) => x.bar); // x is Bar
            of(foobar)
    ...
    it('should not emit errors sent from the source *after* it found the first value in reentrant scenarios', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '   -------(a|)';
            const subs = '       ^------!';
            const result = merge(source, subject).pipe(first(), tap(() => {
    ...
    it('should not emit errors sent from the source *after* it found the first value in reentrant scenarios', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '   ------------(b|)';
            const subs = '       ^-----------!';
            const result = merge(source, subject).pipe(first((value) => value === 'b'), tap(() => {
                subject.error(new Error('reentrant shennanigans'));
            }));
            expectObservable(result).toBe(expected);
            expectSubscriptions(source.subscriptions).toBe(subs);
