...
describe('delay', () => {
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        testScheduler.run(({ hot, time, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '------a--   ';
            const unsub = '   --------!   ';
            const result = e1.pipe(mergeMap((x) => of(x)), delay(t), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should unsubscribe scheduled actions after execution', () => {
        testScheduler.run(({ cold, time, expectObservable, expectSubscriptions }) => {
            let subscribeSpy = null;
            const counts = [];
            const e1 = cold(' a|      ');
            const t = time('  -|      ');
            const expected = '--a-(a|)';
            const result = e1.pipe(repeatWhen((notifications) => {
                const delayed = notifications.pipe(delay(t));
                subscribeSpy = sinon.spy(notifications, 'subscribe');
                return delayed;
            }), skip(1), tap({
                next() {
