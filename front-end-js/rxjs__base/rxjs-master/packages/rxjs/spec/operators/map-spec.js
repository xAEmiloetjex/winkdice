...
describe('map', () => {
    ...
    it('should map with index', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            let invoked = 0;
            const result = e1.pipe(map((x, index) => {
                invoked++;
    ...
    it('should not break unsubscription chain when unsubscribed explicitly', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '--x--y-     ';
            const unsub = '   ------!     ';
            const result = e1.pipe(mergeMap((x) => of(x)), map(addDrama), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected, { x: '1!', y: '2!' });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    describe(/* why u */ 'no lift' /* bro? */, () => {
        it('can operate on any Observable-shaped object', () => {
            ...
            const results = [];
            map((x) => x + '!!!')(source).subscribe((value) => {
                results.push(value);
            });
