...
describe('catchError operator', () => {
    ...
    it('should catch error and replace with a cold Observable', () => {
        testScheduler.run(({ hot, cold, expectObservable }) => {
            ...
            const result = e1.pipe(catchError((err) => e2));
    ...
    it('should not break unsubscription chain when unsubscribed explicitly', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '--1-2-3-         ';
            const unsub = '   -------!         ';
            const result = e1.pipe(mergeMap((x) => of(x)), catchError(() => {
                return of('X', 'Y', 'Z');
            }), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should stop listening to a synchronous observable when unsubscribed', () => {
        const sideEffects = [];
        const synchronousObservable = concat(defer(() => {
            sideEffects.push(1);
            return of(1);
        }), defer(() => {
            sideEffects.push(2);
        ...
        throwError(() => new Error('Some error'))
            .pipe(catchError(() => synchronousObservable), takeWhile((x) => x != 2) // unsubscribe at the second side-effect
        )
            .subscribe(() => {
            /* noop */
        });
    ...
    it('should catch and replace a Observable.throw() as the source', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            const subs = '    (^!)';
            const expected = '(abc|)';
            const result = e1.pipe(catchError((err) => of('a', 'b', 'c')));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(subs);
        });
    });
    it('should mirror the source if it does not raise errors', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(catchError((err) => of('x', 'y', 'z')));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(subs);
        });
    });
    ...
    it('should catch errors throw from within the constructor', () => {
        ...
        testScheduler.run(({ expectObservable }) => {
            const source = concat(new Observable((o) => {
                o.next('a');
                throw 'kaboom';
            }).pipe(catchError((_) => of('b'))), of('c'));
            const expected = '(abc|)';
    ...
    it('Properly handle async handled result if source is synchronous', (done) => {
        ...
        const sourceWithDelay = new Observable((observer) => {
