...
describe('take', () => {
    ...
    it('should not break unsubscription chain when unsubscribed explicitly', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const e1subs = '   ^--------!            ';
            const expected = ' ---a--b---            ';
            const result = e1.pipe(mergeMap((x) => of(x)), take(42), mergeMap((x) => of(x)));
    ...
    it('should unsubscribe from the source when it reaches the limit before a recursive synchronous upstream error is notified', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            const e1subs = '  (^!)';
            const expected = '(a|)';
            const result = merge(e1, subject).pipe(take(1), tap(() => subject.error('error')));
    ...
    it('should unsubscribe from the source when it reaches the limit before a recursive synchronous upstream completion is notified', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            const e1subs = '  (^!)';
            const expected = '(a|)';
            const result = merge(e1, subject).pipe(take(1), tap(() => subject.complete()));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should properly handle reentrancy when taking many values', () => {
        ...
        merge(subject, of(1))
            .pipe(finalize(() => results.push('finalize source')), take(5), tap((n) => subject.next(n + 1)), finalize(() => results.push('finalize result')))
            .subscribe({
