...
describe('switchScan', () => {
    ...
    it('should unsub inner observables', () => {
        const unsubbed = [];
        of('a', 'b')
            .pipe(switchScan((_acc, x) => new Observable((subscriber) => {
            subscriber.complete();
            return () => {
                unsubbed.push(x);
            };
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        testScheduler.run(({ hot, cold, expectObservable, expectSubscriptions }) => {
            ...
            const unsub = '   ---------------------!                ';
            const observableLookup = { x, y };
            const result = e1.pipe(mergeMap((x) => of(x)), switchScan((_acc, value) => observableLookup[value], null), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(x.subscriptions).toBe(xsubs);
            expectSubscriptions(y.subscriptions).toBe(ysubs);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should stop listening to a synchronous observable when unsubscribed', () => {
        ...
        of(null)
            .pipe(switchScan(() => synchronousObservable, null), takeWhile((x) => x != 2) // unsubscribe at the second side-effect
        )
            .subscribe(() => {
            /* noop */
        });
    ...
    it('should create a new seed for each subscriber', () => {
        const seeds = [];
        const observer = (value) => seeds.push(value);
