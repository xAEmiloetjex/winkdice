...
describe('Observable.prototype.concatMap', () => {
    ...
    it('should handle a never source', () => {
        testScheduler.run(({ hot, cold, expectObservable, expectSubscriptions }) => {
            ...
            const innersubs = [];
            const expected = '  -';
            const result = e1.pipe(concatMap(() => {
                return inner;
            }));
            expectObservable(result).toBe(expected);
            expectSubscriptions(inner.subscriptions).toBe(innersubs);
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        testScheduler.run(({ hot, cold, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(mergeMap((x) => of(x)), concatMap((value) => observableLookup[value]), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
            expectSubscriptions(c.subscriptions).toBe(csubs);
    ...
    it('should finalize before moving to the next observable', () => {
        const results = [];
        const create = (n) => defer(() => {
            results.push(`init ${n}`);
            return of(`next ${n}`).pipe(delay(100, testScheduler), finalize(() => {
    ...
    it('should map values to constant resolved promises and concatenate', (done) => {
        ...
        const project = (value) => from(Promise.resolve(42));
        const results = [];
    ...
    it('should map values to constant rejected promises and concatenate', (done) => {
        const source = from([4, 3, 2, 1]);
        const project = (value) => from(Promise.reject(42));
        source.pipe(concatMap(project)).subscribe({
            next: (x) => {
    ...
    it('should map values to resolved promises and concatenate', (done) => {
        const source = from([4, 3, 2, 1]);
        const project = (value, index) => from(Promise.resolve(value + index));
        const results = [];
        source.pipe(concatMap(project)).subscribe({
            next: (x) => {
                results.push(x);
    ...
    it('should map values to rejected promises and concatenate', (done) => {
        const source = from([4, 3, 2, 1]);
        const project = (value, index) => from(Promise.reject('' + value + '-' + index));
        source.pipe(concatMap(project)).subscribe({
            next: (x) => {
                done(new Error('Subscriber next handler not supposed to be called.'));
            },
    ...
    it('should stop listening to a synchronous observable when unsubscribed', () => {
        ...
        synchronousObservable
            .pipe(concatMap((value) => of(value)), take(3))
            .subscribe(() => {
            /* noop */
        });
