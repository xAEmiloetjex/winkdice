...
describe('onErrorResumeNextWith', () => {
    ...
    it('should skip invalid sources and move on', () => {
        const results = [];
        of(1)
            .pipe(onErrorResumeNextWith([2, 3, 4], { notValid: 'LOL' }, of(5, 6)))
            .subscribe({
    ...
    it('should call finalize after each sync observable', () => {
        const results = [];
        of(1)
            .pipe(finalize(() => results.push('finalize 1')), onErrorResumeNextWith(of(2).pipe(finalize(() => results.push('finalize 2'))), of(3).pipe(finalize(() => results.push('finalize 3'))), of(4).pipe(finalize(() => results.push('finalize 4')))))
            .subscribe({
            next: (value) => results.push(value),
            complete: () => results.push('complete'),
        });
    ...
    it('should not subscribe to the next source until after the previous is finalized.', () => {
        ...
        }), onErrorResumeNextWith(of(2).pipe(tap({
