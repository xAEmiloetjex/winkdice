...
describe('takeWhile', () => {
    ...
    it('should take all elements with truthy predicate', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const e1subs = '    ^-------------!';
            const expected = '  --b--c--d--e--|';
            const result = e1.pipe(takeWhile((() => {
                return {};
            })));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should skip all elements with falsy predicate', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const e1subs = '    ^-!            ';
            const expected = '  --|            ';
            const result = e1.pipe(takeWhile(() => null));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should invoke predicate until return false', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(takeWhile(predicate), tap({
                complete: () => {
    ...
    it('should not break unsubscription chain when unsubscribed explicitly', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(mergeMap((x) => of(x)), takeWhile(predicate), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not emit errors sent from the source *after* it found the first value in reentrant scenarios', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '   -------a----(b|)';
            const subs = '       ^-----------!';
            const result = merge(source, subject).pipe(takeWhile((x) => x !== 'b', true), tap({
                complete: () => {
    ...
    it('should not emit errors sent from the source *after* it found the first value in reentrant scenarios in next', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '   -------a----(b|)';
            const subs = '       ^-----------!';
            const result = merge(source, subject).pipe(takeWhile((x) => x !== 'b', true), tap((x) => {
                if (x === 'b') {
                    subject.error(new Error('reentrant shennanigans'));
                }
            }));
    ...
    it('should not emit errors sent from the source *after* it found the first value in reentrant scenarios', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '   -------a----|';
            const subs = '       ^-----------!';
            const result = merge(source, subject).pipe(takeWhile((x) => x !== 'b'), tap({
                complete: () => {
                    subject.error(new Error('reentrant shennanigans'));
                },
            }));
