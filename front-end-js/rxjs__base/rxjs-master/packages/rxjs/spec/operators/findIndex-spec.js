...
describe('findIndex', () => {
    ...
    it('should allow unsubscribing early and explicitly', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '-------     ';
            const unsub = '   ------!     ';
            const result = e1.pipe(findIndex((value) => value === 'z'));
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '-------     ';
            const unsub = '   ------!     ';
            const result = e1.pipe(mergeMap((x) => of(x)), findIndex((value) => value === 'z'), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should unsubscribe when the predicate is matched', () => {
        testScheduler.run(({ hot, time, expectObservable, expectSubscriptions }) => {
            ...
            //                     --|
            const expected = '-------(x|) ';
            const result = e1.pipe(findIndex((value) => value === 'b'), delay(t));
            expectObservable(result).toBe(expected, { x: 1 });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
