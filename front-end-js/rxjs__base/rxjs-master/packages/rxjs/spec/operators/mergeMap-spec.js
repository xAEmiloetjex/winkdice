...
describe('mergeMap', () => {
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        testScheduler.run(({ cold, hot, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '-----i---j---(ki)(lj)(ki)(lj)(ki)(lj)(ki)(lj)k---l---i--';
            const unsub = '   -------------------------------------------------------!';
            const source = e1.pipe(map((x) => x), mergeMap(() => x), map((x) => x));
            expectObservable(source, unsub).toBe(expected);
            expectSubscriptions(x.subscriptions).toBe(xsubs);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    ...
    it('should not break unsubscription chains with interop inners when result is unsubscribed explicitly', () => {
        testScheduler.run(({ cold, hot, expectObservable, expectSubscriptions }) => {
            ...
            // interop subscriber.
            const result = e1.pipe(mergeMap((x) => of(x)), mergeMap((value) => asInteropObservable(observableLookup[value])), mergeMap((x) => of(x)));
    ...
    it('should map and flatten an Array', () => {
        const source = of(1, 2, 3, 4).pipe(mergeMap((x) => [x + '!']));
        const expected = ['1!', '2!', '3!', '4!'];
        let completed = false;
        source.subscribe({
            next: (x) => {
    ...
    it('should stop listening to a synchronous observable when unsubscribed', () => {
        ...
        synchronousObservable
            .pipe(mergeMap((value) => of(value)), take(3))
            .subscribe(() => {
            /* noop */
        });
