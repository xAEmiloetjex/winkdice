...
import { map, mergeMap, mergeMapTo, onErrorResumeNextWith, repeat, retry, share, startWith, take, takeUntil, takeWhile, tap, toArray, withLatestFrom, } from 'rxjs/operators';
        ...
        describe(title, () => {
            ...
            it('should not break unsubscription chain when last subscriber unsubscribes', () => {
                rxTest.run(({ cold, hot, expectObservable, expectSubscriptions }) => {
                    ...
                    const unsub2 = '         ------------!   ';
                    const expected2 = '      --------3----   ';
                    const shared = source.pipe(mergeMap((x) => of(x)), share(options), mergeMap((x) => of(x)));
                    expectObservable(subscriber1.pipe(mergeMapTo(shared)), unsub1).toBe(expected1);
                    expectObservable(subscriber2.pipe(mergeMapTo(shared)), unsub2).toBe(expected2);
                    expectSubscriptions(source.subscriptions).toBe(sourceSubs);
                });
            ...
            it('should not fail on reentrant subscription', () => {
                rxTest.run(({ cold, expectObservable, expectSubscriptions }) => {
                    ...
                    const shared = source.pipe(withLatestFrom(deferred), map(([a, b]) => String(Number(a) + Number(b))), share(options));
                    expectObservable(shared).toBe(expected);
        ...
        describe(title, () => {
            it('should not reset on error if configured to do so', () => {
                rxTest.run(({ hot, expectObservable, expectSubscriptions }) => {
                    ...
                    take(3), share({ resetOnError }), repeat());
            ...
            it('should not reset on complete if configured to do so', () => {
                rxTest.run(({ cold, expectObservable, expectSubscriptions }) => {
                    ...
                    let count = 0;
                    const result = source.pipe(takeUntil(triggerComplete), share({ resetOnComplete }), 
                    // Retry on any error.
                    retry(), tap(() => {
                        if (++count === 9) {
            ...
            it('should not reset on refCount 0 if configured to do so', () => {
                rxTest.run(({ hot, expectObservable, expectSubscriptions }) => {
                    ...
                    const result = source.pipe(tap((value) => {
                        if (value === 'E') {
                            throw new Error('E');
                        }
                    }), takeWhile((value) => value !== 'C'), share({ resetOnRefCountZero }), retry(), repeat());
