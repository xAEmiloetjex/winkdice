...
describe('exhaustMap', () => {
    ...
    it('should raise error if project throws', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const e1subs = '  ^--!';
            const expected = '---#';
            const result = e1.pipe(exhaustMap(() => {
                throw 'error';
            }));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        testScheduler.run(({ cold, hot, expectObservable, expectSubscriptions }) => {
            ...
            const unsub = '   ----------------------------------!           ';
            const observableLookup = { x: x, y: y, z: z };
            const result = e1.pipe(mergeMap((x) => of(x)), exhaustMap((value) => observableLookup[value]), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(x.subscriptions).toBe(xsubs);
            expectSubscriptions(y.subscriptions).toBe(ysubs);
            expectSubscriptions(z.subscriptions).toBe(zsubs);
    ...
    it('should not break unsubscription chains with interop inners when result is unsubscribed explicitly', () => {
        testScheduler.run(({ cold, hot, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(mergeMap((x) => of(x)), exhaustMap((value) => asInteropObservable(observableLookup[value])), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(x.subscriptions).toBe(xsubs);
            expectSubscriptions(y.subscriptions).toBe(ysubs);
            expectSubscriptions(z.subscriptions).toBe(zsubs);
    ...
    it('should stop listening to a synchronous observable when unsubscribed', () => {
        ...
        of(null)
            .pipe(exhaustMap(() => synchronousObservable), takeWhile((x) => x != 2) // unsubscribe at the second side-effect
        )
            .subscribe(() => {
            /* noop */
        });
    ...
    it('should stop listening to a synchronous observable when unsubscribed', () => {
        ...
        synchronousObservable
            .pipe(exhaustMap((value) => of(value)), take(3))
            .subscribe(() => {
            /* noop */
        });
    ...
    it('should ignore subsequent synchronous reentrances during subscribing the inner sub', () => {
        ...
        const results = [];
        e.pipe(take(3), exhaustMap((value) => new Observable((subscriber) => {
            e.next(value + 1);
