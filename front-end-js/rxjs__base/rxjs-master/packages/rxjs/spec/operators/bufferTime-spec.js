...
describe('bufferTime operator', () => {
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        testScheduler.run(({ hot, time, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(mergeMap((x) => of(x)), bufferTime(t, interval, Infinity, testScheduler), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(subs);
        });
    });
    ...
    it('should not mutate the buffer on reentrant next', () => {
        testScheduler.run(({ expectObservable, time }) => {
            ...
            const t2 = time(' --|');
            const expected = '--(a|)';
            const result = subject.pipe(bufferTime(t2), tap(() => subject.next(2)), take(1));
