...
describe('switchMap', () => {
    ...
    it('should unsub inner observables', () => {
        const unsubbed = [];
        of('a', 'b')
            .pipe(switchMap((x) => new Observable((subscriber) => {
            subscriber.complete();
            return () => {
                unsubbed.push(x);
            };
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', () => {
        testScheduler.run(({ hot, cold, expectObservable, expectSubscriptions }) => {
            ...
            const unsub = '   ---------------------!                ';
            const observableLookup = { x: x, y: y };
            const result = e1.pipe(mergeMap((x) => of(x)), switchMap((value) => observableLookup[value]), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(x.subscriptions).toBe(xsubs);
            expectSubscriptions(y.subscriptions).toBe(ysubs);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should not break unsubscription chains with interop inners when result is unsubscribed explicitly', () => {
        testScheduler.run(({ hot, cold, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(mergeMap((x) => of(x)), switchMap((value) => asInteropObservable(observableLookup[value])), mergeMap((x) => of(x)));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(x.subscriptions).toBe(xsubs);
            expectSubscriptions(y.subscriptions).toBe(ysubs);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should stop listening to a synchronous observable when unsubscribed', () => {
        ...
        of(null)
            .pipe(switchMap(() => synchronousObservable), takeWhile((x) => x != 2) // unsubscribe at the second side-effect
        )
            .subscribe(() => {
            /* noop */
        });
    ...
    it('should stop listening to a synchronous observable when unsubscribed', () => {
        ...
        synchronousObservable
            .pipe(switchMap((value) => of(value)), take(3))
            .subscribe(() => {
            /* noop */
        });
    ...
    it('should unsubscribe previous inner sub when getting synchronously reentrance during subscribing the inner sub', () => {
        ...
        const results = [];
        e.pipe(take(3), switchMap((value) => new Observable((subscriber) => {
            e.next(value + 1);
