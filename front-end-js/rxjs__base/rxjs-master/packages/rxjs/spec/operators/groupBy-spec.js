...
describe('groupBy operator', () => {
    ...
    it('should group numbers by odd/even', () => {
        testScheduler.run(({ cold, hot, expectObservable }) => {
            ...
            const source = e1.pipe(groupBy((val) => parseInt(val) % 2));
    ...
    it('should handle an empty Observable', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            const e1subs = '  (^!)';
            const expected = '|   ';
            const source = e1.pipe(groupBy((val) => val.toLowerCase().trim()));
    ...
    it('should emit GroupObservables', () => {
        testScheduler.run(({ cold, hot, expectObservable, expectSubscriptions }) => {
            ...
            const source = e1.pipe(groupBy((val) => val.toLowerCase().trim()), tap((group) => {
    ...
    it('should group values with a keySelector, assert GroupSubject key', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const source = e1.pipe(groupBy((val) => val.toLowerCase().trim()), map((g) => g.key));
            expectObservable(source).toBe(expected, expectedValues);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should allow outer to be unsubscribed early', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const source = e1.pipe(groupBy((val) => val.toLowerCase().trim()), map((group) => group.key));
    ...
    it('should unsubscribe from the source when the outer and inner subscriptions are disposed', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const e1subs = '       ^-!                        ';
            const expected = '     --(a|)                     ';
            const source = e1.pipe(groupBy((val) => val.toLowerCase().trim()), take(1), mergeMap((group) => group.pipe(take(1))));
    ...
    it('should not break unsubscription chain when unsubscribed explicitly', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const unsub = '        -----------!               ';
            const expectedValues = { w: 'foo', x: 'bar', y: 'baz' };
            const source = e1.pipe(mergeMap((x) => of(x)), groupBy((x) => x.toLowerCase().trim()), mergeMap((group) => of(group.key)));
            expectObservable(source, unsub).toBe(expected, expectedValues);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should group values with a keySelector which eventually throws', () => {
        testScheduler.run(({ cold, hot, expectObservable, expectSubscriptions }) => {
            ...
            const expectedValues = { w: w, x: x, y: y, z: z };
            let invoked = 0;
            const source = e1.pipe(groupBy((val) => {
                invoked++;
                if (invoked === 10) {
                    throw 'error';
                }
    ...
    it('should group values with a keySelector and elementSelector, but elementSelector throws', () => {
        testScheduler.run(({ cold, hot, expectObservable, expectSubscriptions }) => {
            ...
            const expectedValues = { w: w, x: x, y: y, z: z };
            let invoked = 0;
            const source = e1.pipe(groupBy((val) => val.toLowerCase().trim(), (val) => {
                invoked++;
                if (invoked === 10) {
                    throw 'error';
                }
    ...
    it('should allow an inner to be unsubscribed early but other inners continue', () => {
        testScheduler.run(({ hot, expectObservable }) => {
            ...
            const source = e1.pipe(groupBy((val) => val.toLowerCase().trim()), map((group) => {
                const arr = [];
    ...
    it('should allow subscribing late to an inner Observable, outer completes', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions, time }) => {
            ...
            const expected = '        ----------------------------|';
            e1.pipe(groupBy((val) => val.toLowerCase().trim())).subscribe((group) => {
                testScheduler.schedule(() => {
    ...
    it('should allow subscribing late to an inner Observable, outer throws', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions, time }) => {
            ...
            const expected = '         ----------------------------#';
            e1.pipe(groupBy((val) => val.toLowerCase().trim())).subscribe({
                next: (group) => {
                    testScheduler.schedule(() => {
                        expectObservable(group).toBe(expected);
                    }, subsDuration);
    ...
    it('should allow using a keySelector, elementSelector, and durationSelector', () => {
        testScheduler.run(({ cold, hot, expectObservable, expectSubscriptions }) => {
            ...
            const source = e1.pipe(groupBy((val) => val.toLowerCase().trim(), (val) => reverseString(val), (group) => group.pipe(skip(2))));
            expectObservable(source).toBe(expected, expectedValues);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should allow using a keySelector, elementSelector, and durationSelector that throws', () => {
        testScheduler.run(({ cold, hot, expectObservable }) => {
            ...
            const z = cold('                         i-----l-|', reversedValues);
            const expectedValues = { v: v, w: w, x: x, y: y, z: z };
            const source = e1.pipe(groupBy((val) => val.toLowerCase().trim(), (val) => reverseString(val), (group) => group.pipe(skip(2), map(() => {
                throw 'error';
            }))));
            expectObservable(source).toBe(expected, expectedValues);
        });
    ...
    it('should allow using a durationSelector, but keySelector throws', () => {
        testScheduler.run(({ cold, hot, expectObservable, expectSubscriptions }) => {
            ...
            const source = e1.pipe(groupBy((val) => {
                ...
                return val.toLowerCase().trim();
            }, (val) => val, (group) => group.pipe(skip(2))));
            expectObservable(source).toBe(expected, expectedValues);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should allow using a durationSelector which eventually throws', () => {
        testScheduler.run(({ cold, hot, expectObservable, expectSubscriptions }) => {
            ...
            let invoked = 0;
            const source = e1.pipe(groupBy((val) => val.toLowerCase().trim(), (val) => val, (group) => {
                invoked++;
                if (invoked === 4) {
                    throw 'error';
                }
    ...
    it('should allow an inner to be unsubscribed early but other inners continue, with durationSelector', () => {
        testScheduler.run(({ cold, hot, expectObservable, expectSubscriptions }) => {
            ...
            const source = e1.pipe(groupBy((val) => val.toLowerCase().trim(), (val) => reverseString(val), (group) => group.pipe(skip(2))), map((group, index) => {
                const arr = [];
                const subscription = group.pipe(phonyMarbelize(testScheduler)).subscribe((value) => {
                    arr.push(value);
                });
    ...
    it('should allow inners to be unsubscribed early at different times, with durationSelector', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const source = e1.pipe(groupBy((val) => val.toLowerCase().trim(), (val) => val, (group) => group.pipe(skip(2))), map((group) => {
                const arr = [];
                const subscription = group.pipe(phonyMarbelize(testScheduler)).subscribe((value) => {
                    arr.push(value);
                });
    ...
    it('should return inners that when subscribed late exhibit hot behavior', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(groupBy((val) => val.toLowerCase().trim(), (val) => val), map((group) => {
                const innerNotifications = [];
                const subscriptionFrame = subscriptionFrames[group.key];
                testScheduler.schedule(() => {
                    group.pipe(phonyMarbelize(testScheduler)).subscribe((value) => {
    ...
    it('should return inner group that when subscribed late emits complete()', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const source = e1.pipe(groupBy((val) => val.toLowerCase().trim(), (val) => val, (group) => group.pipe(skip(7))), map((group) => {
                const arr = [];
                testScheduler.schedule(() => {
                    group.pipe(phonyMarbelize(testScheduler)).subscribe((value) => {
                        arr.push(value);
    ...
    it('should not error for late subscribed inners if outer is unsubscribed before inners are subscribed', () => {
        testScheduler.run(({ hot, expectObservable }) => {
            ...
            const result = source.pipe(groupBy((char) => char), tap({
    ...
    it('should stop listening to a synchronous observable when unsubscribed', () => {
        ...
        synchronousObservable
            .pipe(groupBy((value) => value), take(3))
            .subscribe(() => {
            /* noop */
        });
...
function phonyMarbelize(testScheduler) {
    return (source) => source.pipe(materialize(), map((notification) => {
