...
describe('filter', () => {
    ...
    it('should invoke predicate once for each checked value', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(filter(predicate), tap({
                complete: () => {
    ...
    it('should compose with another filter to allow multiples of six', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const e1subs = '       ^------------------!';
            const expected = '     --------6----------|';
            const result = e1.pipe(filter((x) => +x % 2 === 0), filter((x) => +x % 3 === 0));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should be able to accept and use a thisArg', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(filter(function (x) {
    ...
    it('should be able to use filter and map composed', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(filter((x) => +x % 2 === 0), map((x) => +x * +x));
    ...
    it('should not break unsubscription chain when unsubscribed explicitly', () => {
        testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
            ...
            const expected = '     --3---5----7-       ';
            const unsub = '        ------------!       ';
            const result = e1.pipe(mergeMap((x) => of(x)), filter(isPrime), mergeMap((x) => of(x)));
