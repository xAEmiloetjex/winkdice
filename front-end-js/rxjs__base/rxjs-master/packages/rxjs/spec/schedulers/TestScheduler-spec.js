        ...
        describe('schedulers', () => {
            it('should support animationFrameScheduler, async and asap schedulers', () => {
                ...
                testScheduler.run(({ animate, cold, expectObservable, time }) => {
                    ...
                    const result = mapped.pipe(mergeMap(() => merge(of('a').pipe(delay(0, animationFrameScheduler)), of('b').pipe(delay(tb, asyncScheduler)), of('c').pipe(delay(0, asyncScheduler)), of('d').pipe(delay(0, asapScheduler)))));
            ...
            it('should emit asap notifications before async notifications', () => {
                ...
                testScheduler.run(({ cold, expectObservable }) => {
                    ...
                    const expected = '   ---(ba)---';
                    const result = mapped.pipe(mergeMap((value) => (value === 'a' ? of(value).pipe(delay(1, asyncScheduler)) : of(value).pipe(delay(0, asapScheduler)))));
                    expectObservable(result).toBe(expected);
                });
            });
            it('should support intervals with zero duration', () => {
                ...
                testScheduler.run(({ cold, expectObservable }) => {
                    ...
                    const result = mapped.pipe(mergeMap(() => merge(interval(0, asyncScheduler).pipe(mapTo('a'), take(3)), interval(0, asapScheduler).pipe(mapTo('b'), take(3)))));
                    expectObservable(result).toBe(expected);
                });
            });
        });
