...
describe('Subscription', () => {
    ...
    describe('unsubscribe()', () => {
        ...
        it('should unsubscribe from all parents', () => {
            ...
            // separate property.
            expect(a._finalizers).to.have.length(1);
            expect(b._finalizers).to.have.length(1);
            expect(c._finalizers).to.have.length(1);
            d.unsubscribe();
            // When d is unsubscribed, it should remove itself from each of its
            // parents.
            expect(a._finalizers).to.have.length(0);
            expect(b._finalizers).to.have.length(0);
            expect(c._finalizers).to.have.length(0);
        });
    });
});
