...
it('should support index and its type', () => {
    const a = of(1, 2, 3).pipe(every((val, index) => val < 3)); // $ExpectType Observable<boolean>
});
it('should support index and its type', () => {
    const a = of(1, 2, 3).pipe(every((val, index) => index < 3)); // $ExpectType Observable<boolean>
});
it('should infer source observable type in parameter', () => {
    const a = of(1, 2, 3).pipe(every((val, index, source) => val < 3)); // $ExpectType Observable<boolean>
});
it('should support optional thisArg parameter', () => {
    const a = of(1, 2, 3).pipe(every((val, index, source) => val < 3, 'any object')); // $ExpectType Observable<boolean>
});
...
it('should handle the Boolean constructor', () => {
    const a = of(0, '', false, null, undefined, -0, 0n).pipe(every(Boolean)); // $ExpectType Observable<false>
    const b = of(0, '', 'hi there', false, null, undefined, -0, 0n).pipe(every(Boolean)); // $ExpectType Observable<boolean>
    const c = of('test', true, 1, [], {}).pipe(every(Boolean)); // $ExpectType Observable<boolean>
