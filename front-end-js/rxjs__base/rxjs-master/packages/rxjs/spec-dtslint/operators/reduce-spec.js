...
it('should infer correctly for accumulator of type array', () => {
    const a = of(1, 2, 3).pipe(reduce((x, y, i) => x, [])); // $ExpectType Observable<number[]>
});
...
it('should act appropriately with no seed', () => {
    ...
    const a = of(1, 2, 3).pipe(reduce((a, v) => '' + v)); // $ExpectType Observable<any>
...
it('should infer types properly from arguments', () => {
    ...
    const a = reduce(toArrayReducer, []); // $ExpectType OperatorFunction<number, number[]>
});
