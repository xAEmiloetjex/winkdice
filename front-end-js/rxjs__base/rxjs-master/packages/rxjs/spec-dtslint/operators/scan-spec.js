...
it('should infer correctly for accumulator of type array', () => {
    const a = of(1, 2, 3).pipe(scan((x, y, i) => x, [])); // $ExpectType Observable<number[]>
});
...
it('should act appropriately with no seed', () => {
    ...
    const a = of(1, 2, 3).pipe(scan((a, v) => '' + v)); // $ExpectType Observable<any>
...
it('should infer types properly from arguments', () => {
    ...
    const a = scan(toArrayReducer, []); // $ExpectType OperatorFunction<number, number[]>
});
