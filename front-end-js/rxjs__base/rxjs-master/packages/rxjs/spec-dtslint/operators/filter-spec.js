...
it('should support a user-defined type guard', () => {
    const o = of(1, 2, 3).pipe(filter((value) => value < 3)); // $ExpectType Observable<1>
});
it('should support a user-defined type guard with an index', () => {
    const o = of(1, 2, 3).pipe(filter((value, index) => index < 3)); // $ExpectType Observable<1>
});
it('should support a user-defined type guard and an argument', () => {
    const o = of(1, 2, 3).pipe(filter((value) => value < 3, 'hola')); // $ExpectType Observable<1>
});
...
it('should support Boolean as a predicate', () => {
    ...
    const s = of(false).pipe(filter(Boolean)); // $ExpectType Observable<never>
    const t = of(0, -0, 1).pipe(filter(Boolean)); // $ExpectType Observable<1>
    const u = of(0, -0).pipe(filter(Boolean)); // $ExpectType Observable<never>
    const v = of('', "foo", "bar").pipe(filter(Boolean)); // $ExpectType Observable<"foo" | "bar">
    const w = of('').pipe(filter(Boolean)); // $ExpectType Observable<never>
