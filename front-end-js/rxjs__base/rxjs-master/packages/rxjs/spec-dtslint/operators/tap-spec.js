...
it('should accept partial observer', () => {
    const a = of(1, 2, 3).pipe(tap({ next: (x) => { } })); // $ExpectType Observable<number>
    const b = of(1, 2, 3).pipe(tap({ error: (x) => { } })); // $ExpectType Observable<number>
    const c = of(1, 2, 3).pipe(tap({ complete: () => { } })); // $ExpectType Observable<number>
});
it('should enforce type for next observer function', () => {
    const a = of(1, 2, 3).pipe(tap({ next: (x) => { } })); // $ExpectError
});
