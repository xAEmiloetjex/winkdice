...
it('should enforce value type', () => {
    const o = of('foo').pipe(single(((value) => value === 2))); // $ExpectError
});
it('should enforce return type', () => {
    const o = of('foo').pipe(single(value => value)); // $ExpectError
});
it('should enforce index type', () => {
    const o = of('foo').pipe(single(((value, index) => index === '2'))); // $ExpectError
});
it('should enforce source type', () => {
    const o = of('foo').pipe(single(((value, index, source) => value === 'foo'))); // $ExpectError
});
it('should handle Boolean constructor properly', () => {
    const a = of(null, undefined, 0, -0, 0n, '').pipe(single(Boolean)); // $ExpectType Observable<never>
    const b = of(null, undefined, 0, 'test', -0, 0n, '').pipe(single(Boolean)); // $ExpectType Observable<"test">
});
it('should handle predicates that always return false properly', () => {
    const a = of(1, 2, 3, 4).pipe(single(() => false)); // $ExpectType Observable<number>
});
