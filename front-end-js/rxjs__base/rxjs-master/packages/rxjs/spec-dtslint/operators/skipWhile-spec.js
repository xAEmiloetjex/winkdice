...
it('should handle Boolean constructor properly', () => {
    ...
    const a = of(null, undefined, 0, -0, '', 0n, false).pipe(skipWhile(Boolean)); // $ExpectType Observable<false | "" | 0 | 0n | null | undefined>
    const b = of(null, 0, -0, '', 0n, false).pipe(skipWhile(Boolean)); // $ExpectType Observable<false | "" | 0 | 0n | null>
    const c = of(1, 2, 3, '', 0n, false, 4).pipe(skipWhile(Boolean)); // $ExpectType Observable<number | false | "" | 0n>
    const d = of(true, 123, 'HI', {}, []).pipe(skipWhile(Boolean)); // $ExpectType Observable<never>
});
it('should handle predicates that always return true properly', () => {
    const a = of(1, 2, 3, 4).pipe(skipWhile(() => true)); // $ExpectType Observable<never>
});
