import type { AsyncMonad, Alternative, Container, Catamorphism } from "../interfaces/index";
declare const enum MaybeState {
    Just = "Just",
    None = "None"
}
export default class MaybeConstructor<T, S extends MaybeState = MaybeState> implements AsyncMonad<T>, Alternative<T>, Container<T>, Catamorphism<[void, T]> {
    ...
    unwrapOr(x: T): T;
    unwrapOrElse(f: () => T): T;
    fold<C>(mapNone: (value: undefined) => C, mapJust: (r: T) => C): C;
...
export declare const isMaybe: <T>(value: unknown | Maybe<T>) => value is Maybe<T>;
export {};
