...
describe('Option', function () {
    ...
    it('collect', (done) => {
        deepEqual(none().collect(v => v % 2 === 0)(v => v * 2).toArray, [], "collect failed");
        deepEqual(some(2).collect(v => v % 2 === 0)(v => v * 2).toArray, [4], "collect failed");
        done();
    });
    ...
    it('flatMap', (done) => {
        deepEqual(none().flatMap((v) => some(v * 2)).isEmpty, true, "flatMap failed");
        deepEqual(some(2).flatMap((v) => some(v * 2)).get, 4, "flatMap failed");
        deepEqual(some(2).flatMap((v) => some(v * 2)).equals(some(4)), true, "flatMap failed");
        done();
    });
    it('find', (done) => {
