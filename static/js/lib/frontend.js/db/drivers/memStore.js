...
export class InMemoryDatabase {
    ...
    getSubMap(paths) {
        ...
        for (const path of paths.slice(0, -1)) {
            ...
            if (Array.isArray(value)) {
                let newMap = fMap.trueMap();
                newMap.manager.replace(value);
                currentMap = newMap;
            }
            else {
                currentMap = value;
    ...
    insert(table, record) {
        ...
        const tableMap = fMap.trueMap();
        if (dbMap)
            tableMap.manager.replace(dbMap);
