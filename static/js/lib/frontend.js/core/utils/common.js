...
/**
  ...
  * @author "xA_Emiloetjex"
 */
export function mkRandStr2(length, chars) {
    let result = "";
...
////@ts-expect-error//
export let uniqueStore = [];
let last_tried = "";
export function UniqueGen(generator, length, retries_max = 10, retries_count = 0, extraCheckFn = function (result) { return false; }) {
    if (retries_max >= retries_count) {
        ...
        return JSON.stringify(Error(JSON.stringify({ _: "couldn't generate a new value within the allowed limit", last_tried })));
}
/**
 * @function mkRandStr3
 * @param {number} length
 ...
 * ```
 */
export function mkRandStr3(length, chars, retry_max = 10) {
    return UniqueGen((_length) => mkRandStr2(_length, chars), length, retry_max, 0);
}
...
export function getFromArray(Arr, ID) {
    let temp_1;
...
export const idxMap = trueMap();
export function newIndex(namespace) {
    if (!idxMap.has(namespace)) {
        idxMap.set(namespace, 0);
    }
    const idx = idxMap.get(namespace);
    idxMap.set(namespace, idx + 1);
    return idx;
}
